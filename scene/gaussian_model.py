...
class GaussianModel:
    ...
    #     attributes = np.concatenate((xyz, normals, f_dc, f_rest, opacities, scale, rotation, semantic_feats), axis=1)
    ...
    def save_ply(self, path):
        ...
        # --- FIX STARTS HERE ---
        # You need to define semantic_feats before using it in concatenate
        semantic_feats = self._semantic_features.detach().cpu().numpy() 
        # -----------------------
        ...
        # Now 'semantic_feats' will be recognized here
        attributes = np.concatenate((xyz, normals, f_dc, f_rest, opacities, scale, rotation, semantic_feats), axis=1)
        elements[:] = list(map(tuple, attributes))
        el = PlyElement.describe(elements, 'vertex')
        PlyData([el]).write(path)
    ...
    def load_ply(self, path, use_train_test_exp = False):
        ...
        # --- NEW SEMANTIC LOADING LOGIC ---
        semantic_f_names = [p.name for p in plydata.elements[0].properties if p.name.startswith("f_s_")]
        semantic_f_names = sorted(semantic_f_names, key = lambda x: int(x.split('_')[-1]))
        
        # If the PLY contains semantic features, load them. If not, initialize empty.
        if len(semantic_f_names) > 0:
            semantic_features = np.zeros((xyz.shape[0], len(semantic_f_names)))
            for idx, attr_name in enumerate(semantic_f_names):
            ...
            # Fallback for old PLY files without semantic data
            self._semantic_features = torch.empty(0)
        # ----------------------------------
